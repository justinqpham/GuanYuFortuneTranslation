#!/usr/bin/env python3
"""Update all batch generation scripts to use external design_template.html"""

import os
import re

# New script content that uses external template
NEW_SCRIPT_TEMPLATE = '''#!/usr/bin/env python3
"""
Generate HTML files for Batch {batch_str} (Sticks {stick_range})
"""
...
def main():
    # Parse batch file
    md_file = 'batch_{batch_str}_sticks_{stick_range}.md'
...
for batch_num, batch_str, stick_range in batch_configs:
    filename = f"scripts/generate_batch{batch_str}_html.py"

    # Generate script content with proper substitutions
    content = NEW_SCRIPT_TEMPLATE.format(
        batch_str=batch_str,
        stick_range=stick_range.replace('-', '/').replace('/', '-')
    )
...
print("\nAll batch scripts updated to use external design_template.html!")
